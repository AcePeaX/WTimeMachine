...
export async function authenticate(req, res, next) {
    const { globalmessage, signature } = {...req.query,...req.body};
    try {
        ...
        if (!verified) {
            return res
                .status(401)
                .send({ error: "Invalid signature.", state: 2 });
        }
        ...
        if (timeDifference > 120) {
            ...
            return res
                .status(401)
                .send({ error: "Signature expired.", state: 3 });
        }
        ...
        logger.warn({ err }, "Error in authentication middleware");
        res.status(500).send({
